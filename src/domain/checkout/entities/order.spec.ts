...
describe('Order unit tests', () => {
	...
	it('should throw error when quantity is less than 1', () => {
		expect(() => {
			const item = new OrderItem('1', product1.id, product1.name, 100, 0);
			const order = new Order('1', '1123', [item]);
		}).toThrow('Quantity must be greater or equal to 1');
	});

	it('should calculate total', () => {
		const item1 = new OrderItem('1', product1.id, product1.name, 100, 2);
		const item2 = new OrderItem('2', product1.id, product1.name, 200, 2);
